...
console.log("errorDocument", errorDocument);
...
// Create ownership controls to enable ACLs
new aws.s3.BucketOwnershipControls(`${bucketName}-ownership-controls`, {
  bucket: bucket.id,
  rule: {
...
new aws.s3.BucketPolicy("bucket-policy", {
  bucket: bucket.id,
  policy: bucket.id.apply((bucketId) =>
    JSON.stringify({
      Version: "2012-10-17",
      Statement: [
        {
